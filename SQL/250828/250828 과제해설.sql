## 문제 9, sakila DB의 “영화 대여 내역”을 바탕으로 다음 항목을 모두 출력해주세요. (250828 과제)
-- 고객별 대여 순위, 
-- 이전 대여와의 간격, 
-- 다음 대여와의 간격,
...
rental_intervals AS (
	SELECT rental_id, customer_id, rental_date,
		rental_rank, first_rental_date, last_rental_date, rental_percentile_rank, rental_cumulative_dist,rental_group,
        DATEDIFF(next_rental_date,rental_date) AS next_rental_gap,
        DATEDIFF(rental_date,previous_rental_date) AS previous_rental_gap
	FROM rental_data
),
########## 3분할된 그룹내 대여날짜 기준 오름차순 정렬
grouped_rental_rank AS (
	SELECT 
		rental_id, customer_id, rental_date, 
        rental_group,
        ROW_NUMBER() OVER (PARTITION BY customer_id, rental_group ORDER BY rental_date) AS group_rental_ranking
    FROM rental_data
)
######### 세가지 합하기
SELECT 
	R.customer_id, R.rental_date, R.rental_rank, R.previous_rental_gap, R.next_rental_gap, R.first_rental_date,
    R.rental_percentile_rank, R. rental_cumulative_dist, R.rental_group, G.group_rental_ranking
